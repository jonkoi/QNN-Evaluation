...
def train(model, data,
    ...
    with tf.device(device_str):
        ...
        with tf.name_scope('objective'):
            ...
            # loss = 0.5 * tf.reduce_sum(tf.square(tf.cast(yt, tf.float32) - y))
            accuracy = tf.reduce_mean(
                tf.cast(tf.nn.in_top_k(y, yt, 1), tf.float32))
        opt = tf.contrib.layers.optimize_loss(loss, global_step, learning_rate, 'SGD',
    ...
    while epoch != num_epochs:
        if len(Option.lr_schedule) / 2:
          if epoch == Option.lr_schedule[0]:
            ...
            print ('lr: %f -> %f' % (lr_old, Option.lr.eval()))
        curr_step = 0
