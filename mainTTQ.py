...
def train(model, data,
    ...
    while epoch != num_epochs:
        ...
        #     name = var.op.name
        #     if name.find('Wp') > -1:
        #         logging.info("Wp:", var.eval())
        #     if name.find('Wn') > -1:
        #         logging.info("Wn:", var.eval())
        # w_p = tf.get_variable('Wp', collections=[tf.GraphKeys.VARIABLES, 'positives'])
        # w_n = tf.get_variable('Wn', collections=[tf.GraphKeys.VARIABLES, 'negatives'])
        # logging.info("Wp:", w_p.eval())
        # logging.info("Wn:", w_n.eval())
        epoch += 1
