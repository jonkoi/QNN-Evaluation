...
def train(model, train_generator, validation_generator, count1, count2,
          batch_size=128,
    ...
    with tf.device('/cpu:0'):
        with tf.name_scope('data'):
            ...
            x, y_one_hot =
            yt = np.array([[ np.where(r==1)[0][0] for r in y_one_hot ]])
...
def main(argv=None):  # pylint: disable=unused-argument
    if not gfile.Exists(FLAGS.checkpoint_dir):
        ...
        model_file = os.path.join('/home/khoi/BinaryNet.tf/models', FLAGS.model + '.py')
    ...
    train_generator, validation_generator, count1, count2 =dataDir.generate(FLAGS.batch_size)

    train(m.model, train_generator, validation_generator, count1, count2,
