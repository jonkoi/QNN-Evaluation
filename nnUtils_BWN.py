...
def xnor_binarize_weight_2(x):
    g = tf.get_default_graph()
    with ops.name_scope ('BinarizeWeight') as name:
        with g.gradient_override_map({"Sign": "XnorBinarizeGrad"}):
            size = tf.stop_gradient(tf.cast(tf.reduce_prod(tf.shape(x[:, :, :, 0])[:3]), dtype=tf.float32))
            alpha = tf.stop_gradient(tf.reduce_sum(tf.abs(x), [0,1,2]) / size)
            x =tf.clip_by_value(x,-1,1)
            y = tf.sign(x)
            return tf.multiply(y, alpha)

@tf.RegisterGradient("XnorBinarizeGrad")
def grad_fg(op, grad):
    x = op.inputs[0]
    ...
    ds = tf.cast(tf.less_equal(tf.abs(x), 1), tf.float32)
    return 100*tf.multiply(grad, tf.add(tf.cast(1 / n, tf.float32), tf.multiply(alpha, ds)))
...
def xnor_binarize_weight(x, name=None):
...
def xnor_binary_activation(x, name=None):

    # def f(x):
    #     beta = np.abs(x).sum(1,keepdims=True).sum(1,keepdims=True).sum(1,keepdims=True) /x[0, :, :, :].size
    #     y = np.sign(x) * beta
    ...
    #     x = op.inputs[0]
    #     n = tf.reduce_prod(tf.shape(x[0, :, :, :])[:3])
    #     alpha = tf.div(tf.reduce_sum(tf.abs(x), [1, 2, 3], keepdims=True), tf.cast(n, tf.float32))
    #     ds = tf.multiply(x, tf.cast(tf.less_equal(tf.abs(x), 1), tf.float32))
    #     temp = tf.add(tf.cast(1 / n, tf.float32), tf.multiply(alpha, ds))
    #     return tf.multiply(grad, temp)
    #
    ...
    # def df(op, grad):
    #     x = op.inputs[0]
    #     channel_sum = tf.reduce_sum(x, axis=3, keepdims=True)
    #     k = tf.ones([3, 3, 1, 1])
    #     n = 3 * 3 * tf.reduce_prod(tf.shape(x[:, :, :, 0])[:])
    #     K = tf.nn.conv2d(channel_sum, k, strides=[1, 1, 1, 1],padding='SAME')/tf.cast(n, tf.float32)
    #     alpha = tf.cast(tf.less_equal(tf.abs(x), 1), tf.float32) # alpha = (|x| <= 1) * 1
    #     temp = tf.add(tf.cast(1 / n, tf.float32), tf.multiply(alpha, K))
    #     return tf.multiply(grad, temp)
